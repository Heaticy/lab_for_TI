            ...
            else // 还没收到0X0D
            {
                ...
                else
                {
                    if (aRxBuffer[0] == '\n') // 最后一个字符是换行符
                    {
                        ...
                        if (*endPtr == '\0' && value >= 0 && value <= 200000000 && single_sweep == 1)
                        {
                            ...
                            ADC_fft_collect(value);
												...
												{													  
														ADC_init();
														double_sweep = 1;
														uint32_t freqs[200];  // 初始化频率数组，大小为200
														double start = 2;  // 对数扫描起始点，以10为底数，100Hz对应2
														double end = 6;  // 对数扫描终止点，以10为底数，1MHz对应6
														double delta = (end - start) / 199;  // 步长，根据扫描点数计算，注意：分母为总扫描点数减一
														for (int i = 0; i < 200; i++) {
