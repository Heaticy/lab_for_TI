...
u8 CSR_DATA2[1] = {0x40}; // 使能 CH0修改
u8 CSR_DATA3[1] = {0x80}; // 使能 CH1修改

// AD9958参考时钟25MHz,开20倍频，到主频500MHz工作

u8 FR1_DATA[3] = {0x00, 0x00, 0x00}; // 20倍频
...
void Init_AD9958(void)
{
    ...
    Intserve();                                // IO口初始化
    IntReset();                                // AD9958复位
    WriteData_AD9958(FR1_ADD, 3, FR1_DATA, 1); // 写功能寄存器1
    WriteData_AD9958(FR2_ADD, 2, FR2_DATA, 1); // 写功能寄存器1
}
...
// SPI数据写入
void WriteData_AD9958(uint8_t RegisterAddress, uint8_t NumberofRegisters, uint8_t *RegisterData, uint8_t temp)
{
...
void Write_frequence(uint8_t Channel, uint32_t Freq)
{
    ...
    u32 Temp;
    Temp = (u32)Freq * 8.589934592 + offset /2000000.0 * Freq; // 将输入频率因子分为四个字节  8.589934592=(2^32)/500000000
    ...
    if (Channel == 0)
    {
        WriteData_AD9958(CSR_ADD, 1, CSR_DATA2, 1);    // 控制寄存器写入CH0通道
        WriteData_AD9958(CFTW0_ADD, 4, CFTW0_DATA, 1); // CTW0 address 0x04.输出CH1设定频率
    }
    else if (Channel == 1)
    {
        WriteData_AD9958(CSR_ADD, 1, CSR_DATA3, 1);    // 控制寄存器写入CH1通道
        WriteData_AD9958(CFTW0_ADD, 4, CFTW0_DATA, 3); // CTW0 address 0x04.输出CH2设定频率
    }
}
...
void Write_Amplitude(u8 Channel, u16 Ampli)
{
    ...
    if (Channel == 0)
    {
        WriteData_AD9958(CSR_ADD, 1, CSR_DATA2, 1);
        WriteData_AD9958(ACR_ADD, 3, ACR_DATA, 1);
    }
    else if (Channel == 1)
    {
        WriteData_AD9958(CSR_ADD, 1, CSR_DATA3, 1);
        WriteData_AD9958(ACR_ADD, 3, ACR_DATA, 1);
    }
}
...
void Write_Phase(u8 Channel, u16 Phase)
{
    ...
    if (Channel == 0)
    {
        WriteData_AD9958(CSR_ADD, 1, CSR_DATA2, 0);
        WriteData_AD9958(CPOW0_ADD, 2, CPOW0_DATA, 0);
    }
    else if (Channel == 1)
    {
        WriteData_AD9958(CSR_ADD, 1, CSR_DATA3, 0);
        WriteData_AD9958(CPOW0_ADD, 2, CPOW0_DATA, 0);
    }
}

// 设置2个通道的频率
void AD9958_SetFrequency2Channel(u32 Freq0, u32 Freq1)
{
...
// 设置2个通道的幅度
void AD9958_SetAmp2Channel(u32 Amp0, u32 Amp1)
{
...
// 设置2个通道的相位
void AD9958_SetPhase2Channel(u32 Phase0, u32 Phase1)
{
...
// 二通道同频
void AD9958_Set_Freq(u32 Freq)
{
    AD9958_SetFrequency2Channel(Freq, Freq);
}
...
void mode1(uint32_t starthz, uint32_t stophz, uint32_t stephz)
{
    ...
    if (starthz < stophz) // 从低到高
    {
        ...
        {
            AD9958_Set_Freq(starthz + i * stephz);
            //			delay_us(1);
        }
    }
    else // 从高到低
